...
install_certs() {
    ...
    for cert in $(ls ${LOCAL_CERT_DIR}); do
        if [ -n "$1" ] && [ -n "$2" ]; then
            ...
            rootdo cp ${LOCAL_CERT_DIR}/$cert ${1}
            rootdo $2
        fi
    done
...
fedora() {
    install_certs "/etc/pki/ca-trust/source/anchors/" "update-ca-trust"
...
debian() {
    ...
    install_certs "/usr/local/share/ca-certificates/" "update-ca-certificates"
}
